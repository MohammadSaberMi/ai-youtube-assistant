...
app.get('/api/models', authenticateToken, async (req, res) => {
  db.get('SELECT openrouter_api_key FROM users WHERE id = ?', [req.user.userId], async (err, user) => {
    if (err) {
      console.error('Database error:', err.message);
      return res.status(500).json({ error: 'Server error' });
    }
...
app.post('/api/videos', authenticateToken, (req, res) => {
  ...
  db.run(sqlVideo, [youtubeVideoId, title || '', description || '', captions || ''], function (err) {
    ...
    Promise.all(commentPromises)
      ...
      .catch(error => res.status(500).json({ error: 'Failed to save comments' }));
  });
});
...
Question: ${question}`;
    ...
    if (llmResponse.data && llmResponse.data.choices && llmResponse.data.choices.length > 0 && llmResponse.data.choices[0].message) {
      ...
      db.run('INSERT INTO chat_history (user_id, video_id, message, sender) VALUES (?, ?, ?, ?)', [req.user.userId, videoId, question, 'user']);
      db.run('INSERT INTO chat_history (user_id, video_id, message, sender) VALUES (?, ?, ?, ?)', [req.user.userId, videoId, answer, 'ai']);
      res.json({ answer });
    } else {
...
app.get('/api/chat-history/:videoId', authenticateToken, (req, res) => {
  ...
  db.all('SELECT message, sender, timestamp FROM chat_history WHERE user_id = ? AND video_id = ? ORDER BY timestamp ASC', [req.user.userId, videoId], (err, rows) => {
    if (err) {
